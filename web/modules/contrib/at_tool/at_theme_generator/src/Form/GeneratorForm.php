...
class GeneratorForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    if (is_writable(dirname($themes_dir))) {
      $dir_options[$themes_dir] = $this->t('Themes directory');
    }
    $dir_options['public://'] = $this->t('Public files');
    $dir_options['custom'] = $this->t('Custom');
    ...
    if ($at_core == FALSE) {
      ...
      \Drupal::messenger()->addMessage($this->t('<a href="//www.drupal.org/project/at_theme" target="_blank">Adaptivetheme</a> is a required base theme for all generated themes, <a href="//www.drupal.org/project/at_theme" target="_blank">download the latest version for Drupal 8/9</a> and place in the themes directory.'), 'error');
    }
    ...
    // The maximum allowable length of a module or theme name.
      ...
      '#required' => TRUE,
      '#default_value' => '',
      '#description' => $this->t('Enter a unique theme name. Letters, spaces and underscores only. Max length @length chars.', ['@length' => DRUPAL_EXTENSION_NAME_MAX_LENGTH]),
    ];
    ...
    $generate_type_options = [
      'starterkit' => $this->t('Standard kit'),
    ];

    if (!empty($base_themes)) {
      $generate_type_options = [
        'starterkit' => $this->t('Starter kit'),
        'clone' => $this->t('Clone'),
        'skin' => $this->t('Skin'),
      ];
    }
    ...
    $form['generate']['generate_type_description_standard_kit'] = [
      '#type' => 'container',
      '#markup' => $this->t('The Starter kit includes an advanced layout and is designed to fully support the UIKit and Color module (both optional).'),
    ...
    $form['generate']['generate_clone_source'] = [
      '#type' => 'select',
      '#title' => $this->t('Clone source'),
      '#options' => $cloneable_themes,
      '#default_value' => '',
      '#description' => $this->t('Clones are copies of existing themes.'),
      '#states' => [
    ...
    $form['generate']['generate_skin_base'] = [
      '#type' => 'select',
      '#title' => $this->t('Skin base theme'),
      '#options' => $base_themes,
      '#default_value' => '',
      '#description' => $this->t('Skins are sub-sub themes that inherit extension, layout and color settings from their base theme. Select an existing theme as the skins base theme.'),
      '#states' => [
    ...
    $form['generate']['options']['generate_scss'] = [
      '#type' => 'checkbox',
      '#title' => $this->t('SCSS (SASS/Grunt)'),
      '#default_value' => 1,
      '#description' => $this->t('Includes all SCSS partials for the UIKit (user facing styles), Layouts and Layout Plugin partials, Grunt tasks, and Bower/NPM package management. If you have no use for SCSS un-check this option.'),
      '#states' => [
        'visible' => [
          'select[name="generate[generate_type]"]' => [
            ['value' => 'starterkit'],
    ...
    $form['generate']['options']['generate_color'] = [
      '#type' => 'checkbox',
      '#title' => $this->t('Color Module'),
      '#default_value' => 1,
      '#description' => $this->t('Provides basic Color module support so you can modify the color of your theme in theme settings.'),
      '#states' => [
        'visible' => [
          'select[name="generate[generate_type]"]' => [
            ['value' => 'starterkit'],
    ...
    $form['generate']['options']['generate_block_config'] = [
      ...
      '#default_value' => 1,
      '#description' => $this->t('Include configuration for blocks. Un-check this setting if you want your theme to inherit the default themes block configuration.'),
      '#states' => [
        'visible' => [
          'select[name="generate[generate_type]"]' => [
            ['value' => 'starterkit'],
    ...
    $form['generate']['options']['generate_templates'] = [
      ...
      '#default_value' => 0,
      '#description' => $this->t('Include copies of Drupals front end twig templates (page.html.twig is always included regardless of this setting).'),
      '#states' => [
        'visible' => [
          'select[name="generate[generate_type]"]' => [
            ['value' => 'starterkit'],
    ...
    $form['generate']['options']['generate_themesettingsfile'] = [
      '#type' => 'checkbox',
      '#title' => $this->t('theme-settings.php'),
      '#default_value' => 0,
      '#description' => $this->t('Include a theme-settings.php file. Includes skeleton code for the form alter, custom validation and submit functions.'),
      '#states' => [
        'visible' => [
          'select[name="generate[generate_type]"]' => [
            ['value' => 'starterkit'],
    ...
    $form['generate']['options']['generate_layout'] = [
      ...
      '#default_value' => 'flex',
      '#description' => $this->t('All modern browsers including IE11 are compatible with Adaptivetheme\'s Flexbox layouts. If you need to support legacy browsers such as IE8/9 select Floats. See <a href="@caniuse" target="_blank">caniuse.com</a> for more info.', ['@caniuse' => 'http://caniuse.com/#search=flexbox']),
      '#states' => [
        'visible' => [
          'select[name="generate[generate_type]"]' => [
            ['value' => 'starterkit'],
    ...
    $form['generate']['options']['generate_layout']['float_warning'] = [
      '#type' => 'container',
      '#markup' => '<div class="messages messages--warning">' . $this->t('Drag and drop row ordering (weights) is not supported in the Float layout.') . '</div>',
      '#states' => [
        'visible' => [
    ...
    $form['generate']['options']['generate_description'] = [
      ...
      '#default_value' => '',
      '#description' => $this->t('Descriptions are used on the Appearance list page.'),
    ];

    // Version.
    $form['generate']['options']['generate_version'] = [
      '#type' => 'textfield',
      '#title' => $this->t('Version string'),
      '#default_value' => '',
      '#description' => $this->t('Numbers, hyphens and periods only. E.g. 8.x-1.0'),
    ];

    // Directory to save to.
    $form['generate']['options']['generate_directory'] = [
      '#type' => 'select',
      '#title' => $this->t('Save to'),
      '#options' => $dir_options,
      '#description' => $this->t('Select the directory to save the new theme in. If you select "Themes" your new theme will be saved to the same directory as adaptivetheme.'),
    ];

    $form['generate']['options']['generate_directory_files'] = [
      '#type' => 'container',
      '#markup' => '<em>' . $this->t('After saving the theme to the public files directory move it to your themes directory.') . '</em>',
    ...
    $form['generate']['options']['generate_directory_custom'] = [
      ...
      '#title' => $this->t('Custom'),
      '#default_value' => '',
      '#description' => $this->t('Enter the path to save the new theme to, e.g. <code>themes/custom</code>'),
      '#states' => [
    ...
    if (!empty($values['generate']['generate_machine_name'])) {
      ...
      if (array_key_exists($machine_name, $theme_data) == FALSE) {
        ...
        if ($target == 'custom') {
          ...
          if (empty($target)) {
            $form_state->setErrorByName($target_error, $this->t('Custom directory field is empty.'));
          }
        }
        if (!is_dir($target) || !is_writable($target)) {
          $form_state->setErrorByName($target_error, $this->t('The target directory does not exist or is not writable (check permissions and target directory name): <code>:target</code>.', [':target' => $target]));
        }
        ...
        if (!file_exists($source) && !is_readable($source)) {
          $form_state->setErrorByName($source_error, $this->t('The source theme (starter kit, skin base or clone source) can not be found or is not readable:<br /><code>@source</code>', ['@source' => $source]));
        }
      }
    }
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if (!empty($values['generate']['generate_machine_name'])) {
      ...
      if (isset($values['generate']['generate_type']) && !empty($values['generate']['generate_type'])) {
        ...
        \Drupal::messenger()->addMessage($this->t('Bummer, something went wrong. Please try again or contact support.'), 'error');
      }
      ...
      $logger_message = $this->t('A new theme <b>@theme_name</b>, with then machine name: <code><b>@machine_name</b></code>, has been generated.', [
      ...
      \Drupal::messenger()->addMessage(
        $this->t("<p>A new theme <b>@theme_name</b>, with then machine name: <code><b>@machine_name</b></code>, has been generated.</p><p>You can find your theme here: <code>@target</code></p><p>@final_instructions</p>", [
          '@theme_name'         => $friendly_name,
          '@machine_name'       => $machine_name,
          '@target'             => $target,
      ...
      \Drupal::messenger()->addMessage($this->t('Bummer, something went wrong with the machine name, please try again or contact support.'), 'error');
    }
  }
